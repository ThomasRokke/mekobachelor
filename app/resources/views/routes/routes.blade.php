        ...
        <div class="ui segment  ">
                        ...
                        @foreach($orders as $order)
                            <div class="item">
                                ...
                                <div class="content">
                                    ...
                                    <a href="{{ route('routes', ['workshop_id' => $order->stop->workshop->workshop_id]) }}" class="description overflow-dots"><i class="icon wrench"></i> {{ $order->stop->workshop->name }}</a>
                                </div>
            ...
            <div class="main-content">
                ...
                <div class="date-group centered">
                    <a href="{{ route('routes', ['date' => date('Y/m/d', strtotime('-1 day', strtotime($date)))]) }}" class="ui left icon button outline secondary basic">
                    ...
                    <a href="{{ route('routes', ['date' => date('Y/m/d', strtotime('+1 day', strtotime($date)))]) }}" class="ui right icon button outline secondary basic">
                        Neste
                                            ...
                                            @if($driver->started === 1)
                                                ...
                                                @if($driver->optimized_time !== null)
                                                    @if($driver->lunch_break === 1)
                                                        ...
                                                        <div class="description">Tilbake: <i class="icon clock outline"></i>{{ date('H:i', strtotime('+30 minutes', strtotime($driver->optimized_time))) }}**</div>
    ...
    <script>
        var flat = flatpickr("#flatPickr", {
            onChange: function(selectedDates, dateStr, instance){
              console.log(dateStr);
                window.location.href = "http://localhost:8000/routes?date="+dateStr;
            },
            "locale": "no"
